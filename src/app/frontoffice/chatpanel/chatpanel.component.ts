...
export class ChatpanelComponent implements OnInit, OnChanges {
  ...
  ngOnInit(): void {
    ...
    this.auth.payload.subscribe(p => {
      this.isLogIn = (this.auth.getUserFromLocalStorage()!=null)?true:false;
      this.currentUser = new OnlineUserDto(this.auth.getUserFromLocalStorage().userId, this.auth.getUserFromLocalStorage().username);
      this.isLogIn = (this.auth.getUserFromLocalStorage()!=null)?true:false;
      this.colorUtils = new ColorConverter();
      this.currentUser = new OnlineUserDto(this.auth.getUserFromLocalStorage().userId, this.auth.getUserFromLocalStorage().username);
    ...
    this.isLogIn = (this.auth.getUserFromLocalStorage()!=null)?true:false;
    this.colorUtils = new ColorConverter();
    if(this.auth.getUserFromLocalStorage()!=null) {
      this.currentUser = new OnlineUserDto(this.auth.getUserFromLocalStorage().userId, this.auth.getUserFromLocalStorage().username);
    }
